...
-(void)setupSegmentButtons {
    navigationView = [[UIView alloc]initWithFrame:CGRectMake(8,-14,self.view.frame.size.width,self.navigationBar.frame.size.height)];
    
    NSInteger numControllers = [viewControllerArray count];
    
    if (!buttonText) {
         buttonText = [[NSArray alloc]initWithObjects: @"Rooms",@"Zones",@"third",@"fourth",@"etc",@"etc",@"etc",@"etc",nil]; //%%%buttontitle
    }
...
-(void)updateCurrentPageIndex:(NSInteger)newIndex {
    self.currentPageIndex = (int)newIndex;
}
...
-(void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray *)previousViewControllers transitionCompleted:(BOOL)completed {
    if (completed) {
        NSInteger tempCurrentPageIndex = [viewControllerArray indexOfObject:[pageViewController.viewControllers lastObject]];
        [self updateCurrentPageIndex:tempCurrentPageIndex];
    }
}
